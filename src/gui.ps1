...
$scriptDir = (Get-Item (Get-Location).Path).parent.FullName
$ErrorActionPreference= 'silentlycontinue'
$ProgressPreference = 'silentlycontinue'

$host.UI.RawUI.WindowTitle = "${scriptDir} betterCelery - v1.0.5-c"
...
function neofetch {
    ...
    $neoversion = (Invoke-WebRequest -Uri 'https://api.github.com/repos/bCelery/Celery/releases/latest' | ConvertFrom-Json).tag_name
...
function startCelery {
    ...
    Stop-Process -Name "CeleryInject.exe" -Force
...
$newestversion = (Invoke-WebRequest -Uri 'https://api.github.com/repos/bCelery/Celery/releases/latest' | ConvertFrom-Json).tag_name
...
if (Test-Path (Join-Path $localAppData "Celery") -PathType Container) {
    ...
    if ($versiondata[0] -eq $newestversion) {
        ...
        if (($scriptDir -Like "*\AppData\Local\Celery\*")) {
            Write-HostCenter -Message "This installation is from an unsafe-source." -ForegroundColor "Red"
            Write-HostCenter -Message "If you installed from a Folder/File, please run your antivirus immediately!" -ForegroundColor "Red"
            Write-HostCenter -Message "You are now being redirected to the official source." -ForegroundColor "Red"
            echo $scriptDir
